...
- (void)addDymView{
    ...
    self.xfTagsView.infoButtonBlock = ^(UIButton *button) {
        //存入tag 方便获取外部对应数据
        if (button.selected) {
            [weakSelf.resultArray[2] addObject:[NSString stringWithFormat:@"%d", (button.tag - 100)]];
        }else{
            [weakSelf.resultArray[2] removeObject:[NSString stringWithFormat:@"%d", (button.tag - 100)]];
        }
        
        !weakSelf.refreshView ? : weakSelf.refreshView(nil);
