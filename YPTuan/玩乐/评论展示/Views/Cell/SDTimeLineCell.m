...
-(void)drawRect:(CGRect)rect{
    ...
    [path moveToPoint:CGPointMake(60, CGRectGetHeight(rect))];
    [path addLineToPoint:CGPointMake(kScreenWidth-BaseBtnPadding, CGRectGetHeight(rect))];
    path.lineWidth = .5f;
    [COLOR_ededed setStroke];
    [path stroke];
...
- (void)setup
{
    ...
    _nameLable.font = HNW3Font(16);
    _nameLable.textColor = kColorLogin;
...
-(void)setShopCommentModel:(YLShopCommentModel *)shopCommentModel{
    _shopCommentModel = shopCommentModel;
    ///头像
    _iconView.image = IMAGENAME(DefaultImageImage);
    [shopCommentModel.commentImages enumerateObjectsUsingBlock:^(YLMerchantImagesModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        
        if (obj.imageType == YLMerchantOrArtificerImageTypeCover) {
            
            [_iconView hg_setImageIconWithUrlString:obj.compressionImageUrl placeholder:nil];
    ...
    _nameLable.text = shopCommentModel.userName;
    ///内容
    _contentLabel.text = shopCommentModel.content;
...
//    ///图片九宫图
    ...
    _picContainerView.picPathStringsArray = imageURLStringsGroups;
    NLog(@"imageURLStringsGroups =%@",imageURLStringsGroups);
    
    if (shopCommentModel.shouldShowMoreButton) { // 如果文字高度超过60
        _moreButton.sd_layout.heightIs(20);
        _moreButton.hidden = NO;
        if (shopCommentModel.isOpening) { // 如果需要展开
    ...
    if (shopCommentModel.commentImages.count) {
...
//    }
    ...
    _timeLabel.text = [XTools timeAgo:shopCommentModel.createTime];
}
