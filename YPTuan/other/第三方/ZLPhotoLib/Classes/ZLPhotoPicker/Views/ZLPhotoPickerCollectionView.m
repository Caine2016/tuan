...
- (BOOL)validatePhotoCount:(NSInteger)maxCount{
    if (self.selectAssets.count >= maxCount || maxCount < 0) {
        NSString *format = [NSString stringWithFormat:@"最多只能选择%ld张图片",(long)maxCount];
        if (maxCount <= 0) {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
 ...
 //   CNSLog(@"Size of video %d",data.length);

    if (data.length > 1024*1024*10) { //10485760
        [ShowHUD showWarn:NSLocalizedString(@"MoreThan10M", nil)];
        return;
    }
