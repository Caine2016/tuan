...
- (UIButton *)doneBtn{
    if (!_doneBtn) {
        ...
        [rightBtn setBackgroundImage:[self imageWithColor:kColorLogin] forState:UIControlStateNormal];
        [rightBtn setBackgroundImage:[self imageWithColor:COLOR_999] forState:UIControlStateDisabled];

        rightBtn.layer.cornerRadius = 3;
        rightBtn.layer.masksToBounds = YES;
        [rightBtn setTitleColor:KVCBGWhiteColor forState:UIControlStateNormal];
        [rightBtn setTitleColor:KVCBGWhiteColor forState:UIControlStateDisabled];
        rightBtn.enabled = YES;
        rightBtn.titleLabel.font = [UIFont systemFontOfSize:17];
        rightBtn.frame = CGRectMake(5, 2, 80, 40);
...
- (UILabel *)makeView{
    if (!_makeView) {
        ...
        makeView.frame = CGRectMake(0, 0, 15, 15);
        makeView.hidden = YES;
...
- (void) pickerCollectionViewDidSelected:(ZLPhotoPickerCollectionView *) pickerCollectionView deleteAsset:(ZLPhotoAssets *)deleteAssets{
    ...
    if (asset.isVideoType) {
        ...
        CNSLog(@"ZLPhotoAssets %@--%hhd",asset.assetURL,asset.isVideoType);
