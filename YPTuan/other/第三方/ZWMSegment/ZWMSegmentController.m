...
//
//  Created by Ciane on 2017/11/23.
//  Copyright © 2017年 Ciane. All rights reserved.
//
...
+ (instancetype)segmentControllerWithTitles:(NSArray<NSString *> *)titles isDynamic:(BOOL)isDynamic {
    return [[self alloc] initWithFrame:CGRectMake(0, 0, kSCREENBOUNDS.size.width, kSCREENBOUNDS.size.height) titles:titles isDynamic:isDynamic];
}

- (instancetype)initWithFrame:(CGRect)frame titles:(NSArray *)titles isDynamic:(BOOL)isDynamic{
    self = [super init];
    ...
    [self segmentPageSetting:isDynamic];
    [self containerViewSetting];
    
    return self;
}

- (void)segmentPageSetting:(BOOL)isDynamic {
    
    _segmentView = [[ZWMSegmentView alloc] initWithFrame:CGRectMake(0, 0, _size.width, 44) titles:_titles isDynamic:isDynamic ];
