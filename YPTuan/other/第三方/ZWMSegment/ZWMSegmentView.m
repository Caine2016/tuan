...
//
//  Created by Caine on 2017/11/23.
//  Copyright © 2017年 Caine. All rights reserved.
//

#import "ZWMSegmentView.h"

typedef void(^ZWMIndexBlock)(NSUInteger ,UIButton *);
//int const ZWMSegmentHeight = 54;//可根据项目需求设置高度
...
+ (instancetype)segmentViewWithFrame:(CGRect)frame titles:(NSArray<NSString *> *)titles isDynamic:(BOOL)isDynamic {
    return [[self alloc] initWithFrame:frame titles:titles isDynamic:isDynamic];
}

- (instancetype)initWithFrame:(CGRect)frame titles:(NSArray <NSString *>*)titles isDynamic:(BOOL)isDynamic{
    ...
    if (isDynamic){ [self segmentBasicSettingFlush] ;}else{ [self segmentBasicSettingDefault];}
...
- (void)segmentBasicSettingDefault {
//    self.backgroundColor = [UIColor colorWithWhite:1. alpha:0.5];
    ...
    _segmentNormalColor = RGBCOLOR(194, 155, 154);;
    _font = jsFont(20);
...
-(void)segmentBasicSettingFlush{
   // self.backgroundColor = [UIColor colorWithWhite:1. alpha:0.5];
    ...
    _scrollEnabled = YES;
    _showSeparateLine = NO;
    self.frame = CGRectMake((kScreenWidth-150)/2-20, self.frame.origin.y,150, 44);
