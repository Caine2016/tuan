...
- (CGFloat)ajustedHeightWithWidth:(CGFloat)width {
    ...
    if (self.text.length > 0) {
        return [self wy_heightWithWidth:width font:self.font breakMode:self.lineBreakMode];
    }
    return 0;
}
...
- (CGFloat)wy_heightWithWidth:(CGFloat)width font:(UIFont *)font breakMode:(NSLineBreakMode)breakMode {
    ...
    CGRect rect = CGRectIntegral([self.text boundingRectWithSize:CGSizeMake(width, CGFLOAT_MAX) options:options attributes:attributes context:nil]);
