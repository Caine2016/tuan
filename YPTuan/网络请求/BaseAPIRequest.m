...
- (void)successedOnCallingAPI:(BaseAPIResponse *)response
{
    ...
    CNSLog(@"%@:%@", [self.child requestPath],response.responseData);
    NSString* data = response.responseData[@"returnMsg"];
    CNSLog(@"returnMsg = %@",[data stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding]);

    if ([self.child respondsToSelector:@selector(responseClass)]) {
        self.responseData =  [BaseResponse mj_objectWithKeyValues:response.responseData];
        CNSLog(@" self.responseData = %@", self.responseData);
        if (![self.responseData.returnCode isEqualToString:ResponseDataReturnCodeTypeSuccess]) {
            response.msg = self.responseData.returnMsg;
